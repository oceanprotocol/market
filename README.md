...
If you are using the `subgraph` inside `barge` as well and there are not approved tokens inside your local subgraph you should either insert them or harcode the ocean token address from this env variable `process.env.NEXT_PUBLIC_OCEAN_TOKEN_ADDRESS` inside `src/components/Publish/_utils.ts` as feeToken and also set the `baseTokenDecimals` to `18`.
